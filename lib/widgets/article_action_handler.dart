...
class ArticleActionHandler {
  ...
  ArticleActionHandler({required this.context, required this.articles});

  /// 处理置顶操作
  Future<void> handlePin(
    List<String> articleIds,
    Function(Article) onUpdate,
  ) async {
  ...
  static Future<void> togglePinForSingle({
    ...
    // 显示提示
    if (context.mounted) {
      AppToast.showSuccess(updatedArticle.isPinned ? '已置顶' : '已取消置顶');
    }
  }
